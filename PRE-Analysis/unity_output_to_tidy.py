...
ACCESS_TOKEN = 'sl.BnoVtcSa3JkBrr9W4H7XCFocItaMRwb1pTLibaVZ77yoDMVnhhyW_B4hlI9VnnYn3GVXq1MKQGZAwiRUG8KpaP_xjCnbXBsj9ctkclDT-1uEcbx3YML04hYV2Ih8OH689wXv80p1PSZSemKgkF89upQ'
...
def find_ball_position_changes(data):
    ...
    for j in range(data['trial_set'].min(), data['trial_set'].max() + 1):
        for k in range(ver['levelCounter'].min(), ver['levelCounter'].max()+1):
            ...
            if len(sub_vect) < 1:
                sub_vect = pd.DataFrame([[99, None,  k, j]], columns=['row_start', 'time', 'levelCounter', 'trial_set']) ### this is to capture when there is a press button or bug
