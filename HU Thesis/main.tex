...
To build the comparison, I followed the steps outlined in my reference study \parencite{Innes2019ACA, SALTAFOSSI2023108642, Ulrich2007}. Again, it is important to mention that there is only one single modality stimuli ($V$) and this limits posibilities of the RMI comparisons. 

The initial step involved testing the validity of stimulus construction by analyzing and modeling reaction times to uni-modal and bi-modal stimuli. Results were mixed. Overall there was a significat difference between the conditions.  When attempting to identify pairwise differences through a Tukey HSD test, no significant differences were found. Alternatively, when running a GLMM a siginifican difference between visual only ($V$) and  visual-touch congruent ($V=T$) conditions was found. There were no singnificant differences betwen the incongruent ($V \neq T$) and the visual only ($V$) conditions. This results confirmed the well observed RSE in  virtual reality during a ball positioning task and raises questions over the incongruent condition. 

When looking at the RMI we see that the most found RT are located between 3100 ms and 3200 ms. With over one-third of all trials found in this 100 ms. Additionally, when performing a t-tes for all participant the cummulative functions for each condition Gx and Gz values are found significantly different also for this 100 ms period and not before or after. It is important to notice that altough it was not significatn the difference between Gx and Gy, for this period of 100 ms we can describe a potential violation of the RSE principal were a bimodal condition ($V \neq T$) was slower than a unimodal condition ($V$). 

Also descriptively, we can see that the same relation between the visual only ($V$) and the congruent condition ($V=T$) holds throughout the whole range of RT. That is, in accordance with the RSE. That is not the case for RT between the incongruent condition and the visual only condition were the visual only condition is faster than the incongruent condition ($V \neq T$) for the first 100 ms of the trial. In violation of the RSE. Later, the incongruent condition is faster that the unimodal visual only condition; in accordance to what we would expect with RSE. 

\Cite{RSE_FBI} proposes a plausible explanation for the absence of a difference in the RSE between congruent and incongruent conditions. It suggests that the mere simultaneous occurrence of visual and vibrotactile stimuli might suffice to result in multisensory integration, irrespective of whether these multisensory stimuli are functionally linked at a higher level or not (e.g., touch in the hand holding the ball). 

An alternative explanation may have to due with context invariance assumption, which states that the processing of one signal is not affected by another.

I believe the discrepancy observed in the incongruent case is so marginal that the statistical power of the present study might not be sufficient to detect it significantly. Regarding the power of the study, one conclusion is that, due to considerations of ecological validity, each trial in our IVR study spans a longer time frame compared to traditional psychophysics experiments (with a mean duration of 3400 ms as opposed to the typical 200 ms). This extended duration contributes to increased variance and skewness in response times (RT). Moreover, according to the literature, the influence of the cardiac cycle on stimuli ranges from 10 to 40 ms, posing a challenge when investigating its effects in IVR environments. Consequently, the statistical power of future research needs to be increased threefold, considering, for instance, involving 60 participants, especially if an additional condition related to heart cycles is to be included in the analysis.
