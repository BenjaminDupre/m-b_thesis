...
I would like thank to Michael Geabler and Arno Villringer for their support in making this thesis possible. To Max Hellrigel-Holderbaum for his editing and references, Zaynep Akbal and Tim Julian Möller for their knowledge and time and making this study possible. 

This thesis was made possible through the support of Max-Plank Institute by Zaynep Akbal and Arno Villringer (NRO-228), Study-DB (0218807).
...
The call for participants targeted healthy, non-smoking German-speaking individuals between 18 and 30 years old. They were contacted and recruited via e-mail, using the database of Max-Plank CBS for registered participants. Participants were required to be righthanded, German-speaking adults between 18 and 29 years old and without neurological or psychiatric illnesses (e.g. epilepsy). When taking part in the experiment participants were paid 9 € the hour. The experimental session took 2.5 hours on average. The participant came to laboratory during October 2020  the study was conducted in accordance with the declaration of Helsinki. All participants provided informed written consent. 
...
The virtual environment simulated a retangular office with a window. Participants appeared sitted in front of a table. With an initial prompt indicating them to  place their hands over the table. 

In Figure \ref{fig:looks}, two panels are depicted. On the left side of the figure (Panel I), we observe the external view of a participant wearing the head device, ready to commence the IVR Memory-Motor Task. Panel II on the right side of the figure showcases four out of the five steps participants undergo when initiating a trial set. These steps are outlined below:
...
All participants were right-handed and exhibited a significant increase in reported sickness. Preliminary inspection indicated that participants registered in the average cybersickness. Overall results change from leaned towards symptoms being "not present". We performed a paired-samples t-test to assess the cybersickness levels before (Pre) and after (Post) the Virtual Reality Experience. There was a significant difference in the scores between the Pre (M=1.10, SD=0.12) and Post (M=1.18, SD=0.19) conditions; $t(21)=-2.65$, $p = 0.015$. On average, the symptoms moved from "not present" to "somewhat present". 
...
\begin{table}[!ht]
    ...
    \end{tabular}}
    \captionsetup{justification=justified, margin={2cm,2cm}, font={small}}
    \caption{Let $G_x$ be the individual CDF estimate of the visual-only condition, and $F_y$ and $F_z$ denote the redun-
    dant CDF for the incongruent and congruent conditions. Each row represents a percentile. Each t-test evaluates a significant difference compare to $G_x$, * indicates significant p-values.}
