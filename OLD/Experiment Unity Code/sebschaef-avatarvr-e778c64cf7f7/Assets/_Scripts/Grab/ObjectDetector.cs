...
public class ObjectDetector : MonoBehaviour
{
    ...
    private GameObject thumb, ring, index;
    /// <summary>
    ...
    private string hand;
    private TwoHandsObjectDetector twoHandsObjectDetector;
    ...
    void Start()
    {
        ...
        hand = (gameObject.name.Contains("hand_r")) ? TwoHandsObjectDetector.RIGHT_HAND : TwoHandsObjectDetector.LEFT_HAND;
        twoHandsObjectDetector = TwoHandsObjectDetector.GetInstance();

        if (hand == TwoHandsObjectDetector.RIGHT_HAND)
        {
            twoHandsObjectDetector.rightPalmCenter = palmCenter;
        }
        else
        {
            twoHandsObjectDetector.leftPalmCenter = palmCenter;
        }
    }
    ...
    public void SetFinger(string name, GameObject value)
    {
        if (name == "thumb")
            thumb = value;
        else if (name == "index")
    ...
    void FixedUpdate()
    {
        ...
        if (cols.Length >= 1)
        {
            twoHandsObjectDetector.SetObjectTouched(hand, cols[0].gameObject);

            if (twoHandsObjectDetector.IsGrabbingWithBothHands())
            {
                twoHandsObjectDetector.SetObjectCaught();
            }
            else
            {
                twoHandsObjectDetector.releaseObject();
                // Gets the objectCaught
                objectCaught = cols[0].gameObject;
                int objectId = objectCaught.GetInstanceID();

                if (objectCaught.GetComponent<ObjectMovement>() == null)
                {
                    // Adds ObjectMovement to calculate the object launching direction and velocity
                    objMov = objectCaught.AddComponent<ObjectMovement>();
                }


                // If the thum and the ring are in contact with the object, perform the grabbing action
                if (thumb != null && objectId.Equals(thumb.GetInstanceID()) &&
                    ring != null && objectId.Equals(ring.GetInstanceID()))
                {
                    objectCaught.transform.SetParent(palmCenter);
                    objectCaught.GetComponent<Rigidbody>().isKinematic = true;
                    objectCaught.transform.localPosition = Vector3.Lerp(objectCaught.transform.localPosition, new Vector3(0f, 0f, -cols[0].bounds.extents.z), Time.deltaTime * 5f);
                }
                else if (objectCaught.GetComponent<Rigidbody>().isKinematic)
                {
                    releaseObject();
                }
            }
        }
        else if (objectCaught != null)
        {
            twoHandsObjectDetector.SetObjectTouched(hand, null);
            twoHandsObjectDetector.releaseObject();
            releaseObject();
        }
        else
        ...
        isGrabbing = objectCaught != null && objectCaught.GetComponent<Rigidbody>().isKinematic;
    }
    ...
    void releaseObject()
    {
        ...
        if (objMov != null)
        {
            ...
            objectCaught.GetComponent<Rigidbody>().AddForce(impulse * objMov.objectSmoothVelocity, ForceMode.Impulse);
            Destroy(objMov);
        }
    ...
    /// </summary>
    /// <returns></returns>
    public IEnumerator DisableEnableColliders()
    {
