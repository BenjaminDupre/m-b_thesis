...
public class NDDevicesInit : MonoBehaviour
{
...
#if VIVE_TRACKERS
    private ChooseTracker chooseTracker;
#endif
    ...
    {
#if VIVE_TRACKERS
        chooseTracker = GameObject.FindObjectOfType<ChooseTracker>();
#endif
...
#if VIVE_TRACKERS
            if (chooseTracker != null && chooseTracker.enabled)
            {
                    ...
                    (leftDevices == 0 && leftHmcs == 0 && rightHmcs == 0 && rightDevices == 1) || (leftDevices == 1 && leftHmcs == 0 && rightHmcs == 0 && rightDevices == 1))
                {
                    ...
                    message = "<b>Shake your hand</b> until the tracker is assigned";
                }
                else
                {
                    message += ".\nThen, <b>shake this hand</b> until the tracker is assigned";
                }
            }
#endif
        }
    ...
    void Update()
    {
        ...
        if (!allControllersAssigned)
        {
            ...
            foreach (HapticController c in controllers)
            {
                ...
                if (c != null && !IsAssigned(c.DeviceId) && c.AreContactsJoined(Contact.CONT_THUMB, Contact.CONT_INDEX) > 0)
                {
                    ...
                    if (c.Location == currentGloveLocation)
                    {
                        ...
                        found = true;
#if VIVE_TRACKERS
                        if (chooseTracker != null)
                            chooseTracker.waitForPinch = true;
                        else
                            chooseTracker.waitForPinch = false;
#endif
                        break;
                    }
                }
            ...
            {
#if VIVE_TRACKERS
                if (chooseTracker != null && chooseTracker.trackerFound)
                {
                    if (assignedDeviceId != -1)
                    {
                        devicesInScene[currentIndex].AssignGlove(assignedDeviceId);
                        Debug.Log("Set Assigned: " + assignedDeviceId);
                        ShowHideRoboticMesh shrm = devicesInScene[currentIndex].hmc.GetComponentInParent<ShowHideRoboticMesh>();
                        if (shrm)
                            shrm.ShowHandByLocation(currentGloveLocation);                    }
                    else
                    {
                        devicesInScene[currentIndex].SetIsAssigned(true);
                    }
                    chooseTracker.ActivateExternalMuxProvider(devicesInScene[currentIndex].hmc);
                    chooseTracker.AssignIKTarget(currentGloveLocation);
                    currentIndex++;
                    WaitForDevice();
                    chooseTracker.trackerFound = false;
                }
#else
                if (assignedDeviceId != -1)
                {
