...
public class HandModelController : MonoBehaviour, IComparable<HandModelController>
{
    ...
    /// <param name="thumbFlex">Thumb flex value</param>
    private void AddAvatarSensorListener(Quaternion[] quat, float thumbFlex)
    {
        UpdateSourceDict(NDDevice.RAW_SOURCE_TAG, CalculatePhalangesLimits(quat, thumbFlex));
        canGetData = true;
    }
    ...
    /// <param name="thumbFlex">Thumb flex value from 0 to 1</param>
    /// <returns>Matrix of Quaternions, where: Quaternion[i][j],
    /// "i" represents the bone, and "j" the component Y or X</returns>
    private Quaternion[][] CalculatePhalangesLimits(Quaternion[] qIn, float thumbFlex = -1)
    {
        Quaternion[][] qAux = new Quaternion[numberOfBones][];
        for (int i = 0; i < qAux.Length; i++)
        {
        ...
        for (int i = 0; i < fingers.Length; i++)
        {
            ...
            if (fingers[i] != null)
            {
                ...
                {
                    case NDModelAxesChanger.Parts.thumb0:
                        q = fingers[i].RotateBone(qIn[i], qIn[i + 1], thumbFlex);
                        qAux[(int)SensorID.Thumb0] = q[0];
                        qAux[(int)SensorID.Thumb1] = q[1];
                        qAux[(int)SensorID.Thumb2] = q[2];
                        break;
